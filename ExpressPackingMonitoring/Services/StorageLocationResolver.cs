...
internal static class StorageLocationResolver
{
    ...
    {
        return ResolveRecordingPlan(config, allowDefaultFallback).WorkingRootPath;
    }

    public static RecordingStoragePlan ResolveRecordingPlan(AppConfig config, bool allowDefaultFallback)
    {
        ...
        if (locations.Count == 0)
        {
            ...
            return new RecordingStoragePlan(defaultPath, defaultPath, false);
        }
        ...
        foreach (StorageLocation location in locations)
        {
            ...
            if (IsNetworkPath(configuredPath))
            {
                ...
                if (!buffer.CanUse)
                    throw new IOException($"本地录像缓冲目录不可用：{buffer.Reason}");
