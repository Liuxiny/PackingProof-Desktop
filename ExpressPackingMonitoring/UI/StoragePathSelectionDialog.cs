...
namespace ExpressPackingMonitoring.UI
{
    public sealed class StoragePathSelectionDialog : Window
    {
        ...
        public StoragePathSelectionDialog(string? initialPath = null)
        {
            ...
            {
                Text = @"可以选择本地磁盘中的任意文件夹，也可以输入网络共享路径，例如：\\server\share\快递打包视频",
                FontSize = 12,
        ...
        internal static bool TryNormalizePath(string? path, out string normalizedPath, out string errorMessage)
        {
            try
            {
                ...
                {
                    normalizedPath = "";
                    errorMessage = @"请输入完整路径，例如 D:\录像 或 \\server\share\录像";
                    return false;
                }
        ...
        private void BrowseButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Description = "选择录像保存文件夹",
                UseDescriptionForTitle = true,
                ShowNewFolderButton = true
            };

            if (TryNormalizePath(_pathTextBox.Text, out string initialPath, out _) && Directory.Exists(initialPath))
        ...
        private void OkButton_Click(object? sender, RoutedEventArgs e)
        {
            ...
            {
                AppDialog.ShowMessage(this, errorMessage, "路径无效", AppDialogSeverity.Warning);
                return;
            }
